...
- 后台新增文章：管理员后台 -> AI 提取/手动录入 -> 人民网 URL 栏目/文章类型纠偏 + 标题/来源/摘要/正文领域评分 + 共享解读格式清洗 -> 保存 articles -> 保存 article_details
...
- src/services/kimiArticleService.ts：文章提取与提取结果格式清洗，现包含人民网 URL 栏目识别、文章类型兜底与领域关键词评分纠偏
