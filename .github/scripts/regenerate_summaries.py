...
REGENERATE_ALL = os.environ.get('REGENERATE_ALL', 'false').lower() == 'true'
...
def get_articles_missing_details():
    """获取需要处理的文章：1.缺少摘要/解读的 2.旧格式解读需要重新生成的"""
    if not SUPABASE_URL:
        return []
    ...
    if details_resp.status_code == 200:
        for d in details_resp.json():
            details_map[d.get('id')] = {'abstract': d.get('abstract', '') or '', 'analysis': d.get('analysis', '') or ''}
    missing, old_format = [], []
    for a in articles:
        aid = a.get('id')
        if not aid: continue
        detail = details_map.get(aid, {})
        abstract, analysis = detail.get('abstract', ''), detail.get('analysis', '')
        has_abstract = len(abstract) > 50 and '正在整理' not in abstract
        has_analysis = len(analysis) > 100 and '正在整理' not in analysis and '生成失败' not in analysis
        if not has_abstract or not has_analysis:
            missing.append(a)
        elif REGENERATE_ALL:
            old_format.append(a)
        elif has_analysis and '一、政治高度' not in analysis and '一、' not in analysis:
            old_format.append(a)
    print(f'[Debug] 缺少摘要/解读: {len(missing)} 篇')
    print(f'[Debug] 旧格式需重新生成: {len(old_format)} 篇')
    result = missing + old_format
    print(f'[Filter] 总计需要处理: {len(result)} 篇')
    return result
...
def generate_summary_and_analysis(title, content, existing_summary=''):
    if not DS_API_KEY: raise Exception('DS_API_KEY 未配置')
...
要求：全中文，禁止JSON格式。'''
    for retry in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 429: time.sleep(30 * (retry + 1)); continue
            if resp.status_code != 200: raise Exception(f'API错误: {resp.status_code}')
...
def main():
    ...
    if not SUPABASE_URL or not DS_API_KEY: print('[Error] 配置不完整'); return
    articles = get_articles_missing_details()
    if not articles: print('[Info] 没有需要处理的文章'); return
    success, fail = 0, 0
    for i, a in enumerate(articles):
        ...
        try:
            ...
            if save_article_detail(a['id'], result['summary'], result['analysis'], content):
                print('  [OK] 保存成功'); success += 1
            else: print('  [Fail] 保存失败'); fail += 1
            time.sleep(REQUEST_DELAY)
        except Exception as e: print(f'  [Fail] {e}'); fail += 1; time.sleep(30)
