...
FILTER_RULES = {
    ...
    'non_direct_xi_patterns': [
        ...
        r'^[李强赵乐际王沪宁蔡奇丁薛祥李希韩正][^，。]*会见',
        r'^[李强赵乐际王沪宁蔡奇丁薛祥李希韩正][^，。]*在.{0,10}(会见|出席|访问)',
        r'^[李强赵乐际王沪宁蔡奇丁薛祥李希韩正][^，。]*分别',
        r'王毅[^，。]{0,10}(会见|访问|出席|在)',
...
def detect_domain(title: str, category: str = '') -> str:
    ...
    if any(kw in normalized_title for kw in DOMAIN_KEYWORDS['diplomacy']):
        return 'diplomacy'

    # 外国领导人名字匹配（去"习近平"后标题中仍有外国领导人名字 = 外交）
    FOREIGN_LEADER_NAMES = [
        '普京', '拜登', '特朗普', '奥巴马', '马克龙', '朔尔茨', '岸田', '尹锡悦', '金正恩',
        '阮富仲', '苏林', '莫迪', '苏纳克', '斯塔默', '米歇尔', '冯德莱恩', '古特雷斯',
        '泽连斯基', '内塔尼亚胡', '埃尔多安', '卢拉', '拉马福萨',
    ]
    if any(name in normalized_title for name in FOREIGN_LEADER_NAMES):
        return 'diplomacy'

    # 外国国家名匹配（标题含具体外国国名 = 外交）
    ...
    if any(name in normalized_title for name in FOREIGN_COUNTRY_NAMES):
        return 'diplomacy'
...
def search_xinhua_mrdx() -> List[Dict]:
    ...
    def maybe_add_article(title: str, url: str, context_text: str, base_url: str = ''):
        ...
        if not clean_title or len(clean_title) < 8 or not full_url:
            return
        if '习近平' not in clean_title and '总书记' not in clean_title:
            if '习近平主席' not in clean_title and '国家主席' not in clean_title:
                return
