...
def fix_source_from_url(url: str, fallback: str = '官方媒体') -> str:
    url = (url or '').lower()
    if 'qstheory.cn' in url:
        return '求是网'
    if 'people.com.cn' in url or 'jhsjk.people.cn' in url:
        return '人民日报'
    if 'paper.people.com.cn' in url:
        return '人民日报'
    if 'mrdx.cn' in url:
        return '新华每日电讯'
    if 'xinhuanet.com' in url or 'news.cn' in url:
        return '新华社'
    if 'gov.cn' in url:
        return '中国政府网'
    if 'cctv.com' in url or 'cntv.cn' in url:
        return '央视网'
    return fallback
...
def search_with_qwen(query: str) -> List[Dict]:
    ...
    try:
        ...
        if not choices:
            print(f'[Qwen] No choices in response')
            return []
        ...
        for round_num in range(3):
            ...
            if tool_calls and finish_reason == 'tool_calls':
                ...
                for tc in tool_calls:
                    messages.append({'role': 'tool', 'content': json.dumps({'result': 'web_search_completed'}), 'tool_call_id': tc.get('id', '')})
                tools = None
                continue
            if content:
...
def search_with_baidu(queries: List[str]) -> List[Dict]:
    print('[Baidu] Starting (fallback mode)...')
    articles = []
        ...
        try:
            ...
            for result in soup.select('.result.c-container')[:8]:
                ...
                articles.append({
                    ...
                    'source': fix_source_from_url(title_elem.get('href', ''), site.split('.')[0]),
...
def search_people_jhsjk() -> List[Dict]:
    ...
    try:
        ...
        for li in soup.select('li'):
            ...
            full_url = f'http://jhsjk.people.cn{href}' if href.startswith('/') else href
            articles.append({'title': title, 'url': full_url, 'source': '人民网', 'date': article_date, 'summary': title})
...
def search_qstheory() -> List[Dict]:
    print('[QiuShi] Starting direct crawl...')
    articles = []
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    try:
        from bs4 import BeautifulSoup
    ...
    urls_to_check = ['https://www.qstheory.cn/', 'https://www.qstheory.cn/dt/']
    seen_urls = set()
        ...
        try:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if not title or len(title) < 8:
                    continue
                is_original = '※习近平' in title or title.startswith('习近平：')
                if not is_original:
                    if '习近平' not in title and '总书记' not in title:
                        continue
                    continue
                ...
                articles.append({'title': clean_title, 'url': full_url, 'source': '求是网', 'date': article_date, 'summary': clean_title})
        except Exception as e:
            print(f'[QiuShi] Error: {e}')
        ...
        try:
            ...
            for a_tag in soup.find_all('a'):
                ...
                if is_non_original_title(title):
                    continue
                if href.startswith('/'):
                    full_url = f'http://mrdx.cn{href}'
                elif href.startswith('http'):
                    full_url = href
                else:
                    full_url = requests.compat.urljoin(issue_url, href)
                if full_url in seen_urls:
                    continue
                seen_urls.add(full_url)
                articles.append({'title': title, 'url': full_url, 'source': '新华社', 'date': valid_dates[0], 'summary': title})
        except Exception as e:
...
def search_rmrb() -> List[Dict]:
    print('[RMRB] Starting direct crawl...')
    articles = []
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    try:
        from bs4 import BeautifulSoup
        ...
        for node in ['node_01.html', 'node_02.html', 'node_03.html', 'node_04.html']:
            ...
            try:
                ...
                for a_tag in soup.find_all('a', href=True):
                    ...
                    if 'content_' not in full_url or full_url in seen_urls:
                        continue
                    seen_urls.add(full_url)
                    articles.append({'title': title, 'url': full_url, 'source': '人民日报', 'date': target_date.strftime('%Y-%m-%d'), 'summary': title})
            except Exception as e:
                print(f'[RMRB] Error: {e}')
    print(f'[RMRB] Total: {len(articles)} articles found')
    return articles


def merge_and_dedupe(baidu_articles, people_articles=None, qstheory_articles=None, xinhua_articles=None, rmrb_articles=None):
    all_articles = []
    seen_titles = set()
    seen_urls = set()
    ...
    non_direct_rejected = []
    kept_articles = []
    if people_articles is None: people_articles = []
    if qstheory_articles is None: qstheory_articles = []
    if xinhua_articles is None: xinhua_articles = []
    if rmrb_articles is None: rmrb_articles = []
    source_counts = {'xinhua': len(xinhua_articles or []), 'qstheory': len(qstheory_articles or []), 'rmrb': len(rmrb_articles or []), 'baidu': len(baidu_articles or []), 'people': len(people_articles or [])}
    ...
    def add(article, source_tag):
        ...
        kept_articles.append({'title': title, 'url': url, 'source': corrected_source})

    for a in xinhua_articles: add(a, 'xinhua')
    for a in qstheory_articles: add(a, 'qstheory')
    for a in rmrb_articles: add(a, 'rmrb')
    for a in baidu_articles: add(a, 'baidu')
    for a in people_articles: add(a, 'people')
    ...
    print(f'[Merge] 输入: {sum(source_counts.values())}, 去重保留: {len(kept_articles)}, 标题重复: {len(duplicate_seen_title)}, 链接重复: {len(duplicate_seen_url)}, 校验淘汰: {len(validation_rejected)}, 非直接相关淘汰: {len(non_direct_rejected)}')
...
def get_existing_articles():
    ...
    for table_name in ['pending_articles', 'articles']:
        try:
            resp = requests.get(f'{SUPABASE_URL}/rest/v1/{table_name}?select=url,title&limit=1000', headers=headers, timeout=10)
            if resp.status_code != 200:
                continue
            for row in resp.json():
                ...
                if title:
                    simple = simplify_title(title)
                    if simple and simple not in existing_titles: existing_titles[simple] = title
        except Exception as e:
            print(f'[Existing] Error: {e}')
    print(f'[Existing] Loaded {len(existing_urls)} urls, {len(existing_titles)} simplified titles')
    return {'urls': existing_urls, 'titles': existing_titles}


def titles_look_duplicate(title, existing_simple_map):
    simple = simplify_title(title)
    if not simple: return ''
    exact = existing_simple_map.get(simple)
    if exact: return exact
    if len(simple) < 8: return ''
    for existing_simple, existing_title in existing_simple_map.items():
        if len(existing_simple) < 8: continue
        if simple in existing_simple or existing_simple in simple: return existing_title
    return ''


def save_articles(articles):
    if not articles or not SUPABASE_URL:
        print('[Save] No articles or no SUPABASE_URL')
        return 0
    safe_articles = []
    for a in articles:
        safe_a = {k: v for k, v in a.items() if k not in ('domain', 'domain_name')}
        safe_articles.append(safe_a)
    try:
        resp = requests.post(
            f'{SUPABASE_URL}/rest/v1/{TABLE}',
            headers={'apikey': SUPABASE_KEY, 'Authorization': f'Bearer {SUPABASE_KEY}', 'Content-Type': 'application/json', 'Prefer': 'return=minimal'},
            json=safe_articles, timeout=30
        )
        if resp.status_code in (200, 201):
            print(f'[Save] Successfully saved {len(safe_articles)} articles')
            return len(safe_articles)
        else:
            print(f'[Save] ERROR: status={resp.status_code}, body={resp.text[:500]}')
            return 0
    except Exception as e:
...
def get_search_pipeline_label(search_type):
    ...
    if search_type == 'auto':
        return f'自动定时搜索（{"8:00 搜昨日" if beijing_hour < 12 else "20:00 搜今日"}）'
    return '手动触发搜索'
...
def save_log(baidu_count, people_count, qstheory_count, xinhua_count, rmrb_count, new_count, status, details):
    ...
    try:
        ...
        log_data = {
            ...
            'new_count': new_count,
            'status': status,
            'details': {**details, 'search_type': search_type, 'api_used': get_search_pipeline_label(search_type)},
            'duration_seconds': 0,
        }
        resp = requests.post(
        ...
        if resp.status_code not in (200, 201):
            print(f'[Log] ERROR: {resp.status_code}')
    except Exception as e:
...
def main():
    ...
    log_details = {**merge_info, 'search_date': search_date, 'target_date': target_date, 'existing_url_filtered': existing_url_filtered, 'existing_url_filtered_count': len(existing_url_filtered), 'final_new_articles': final_new_titles, 'save_result': save_result}
    save_log(len(baidu_articles), len(people_articles), len(qstheory_articles), len(xinhua_articles), len(rmrb_articles), saved, 'success', log_details)
    print(f'=== Done: Xinhua {len(xinhua_articles)}, RMRB {len(rmrb_articles)}, People {len(people_articles)}, QiuShi {len(qstheory_articles)}, Baidu {len(baidu_articles)}, Merged {len(merged)}, ExistingFiltered {len(existing_url_filtered)}, New {saved} ===')
