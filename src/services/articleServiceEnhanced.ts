...
import staticArticlesData from '@/data/articles.json';
import {
...
const staticArticles: Speech[] = (staticArticlesData as unknown as Speech[]).map(a => ({
  ...a,
  summary: normalizeSummaryText(a.summary || ''),
...
// 获取所有文章（静态数据优先，不查 Supabase 节省带宽）
export async function getArticles(): Promise<Speech[]> {
  if (staticArticles.length > 0) {
    return staticArticles.filter(a => !a.isZhengjiguan);
  }
  return getLocalCache().filter(a => !a.isZhengjiguan);
}

// 获取政绩观专题文章（静态数据优先，不查 Supabase）
export async function getZhengjiguanArticles(): Promise<Speech[]> {
  const zgArticles = staticArticles.filter(a => a.isZhengjiguan);
  if (zgArticles.length > 0) {
    return zgArticles.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }
  return [];
}

// 同步获取文章（静态数据优先，用于首屏快速加载）
