...
const PREFERRED_EXTRACTION_API_STORAGE = 'preferred_extraction_api';
...
export function getPreferredExtractionApi(): 'kimi' | 'deepseek' {
  return (localStorage.getItem(PREFERRED_EXTRACTION_API_STORAGE) as 'kimi' | 'deepseek') || 'deepseek';
}
...
/**
 * 根据标题检测文章领域（与 Python detect_domain 对齐）
 */
const FOREIGN_LEADER_NAMES = [
  '普京', '拜登', '特朗普', '奥巴马', '马克龙', '朔尔茨', '岸田', '尹锡悦', '金正恩',
  '阮富仲', '苏林', '莫迪', '苏纳克', '斯塔默', '米歇尔', '冯德莱恩', '古特雷斯',
  '泽连斯基', '内塔尼亚胡', '埃尔多安', '卢拉', '拉马福萨',
];
const FOREIGN_COUNTRY_NAMES = [
  '俄罗斯', '美国', '日本', '韩国', '朝鲜', '越南', '印度', '英国', '法国', '德国',
  '澳大利亚', '巴西', '加拿大', '意大利', '西班牙', '墨西哥', '印度尼西亚', '沙特',
  '伊朗', '土耳其', '以色列', '乌克兰', '南非', '阿根廷', '泰国', '马来西亚', '新加坡',
  '菲律宾', '缅甸', '柬埔寨', '老挝', '蒙古国', '巴基斯坦', '孟加拉国', '埃及', '尼日利亚',
  '肯尼亚', '坦桑尼亚', '古巴', '委内瑞拉', '智利', '秘鲁', '哥伦比亚',
  '东盟', '阿盟', '非盟', '欧盟', '亚太经合组织', '金砖', '上合', 'G20', 'G7',
];
const DOMAIN_KEYWORDS_MAP: Record<string, string[]> = {
  diplomacy: ['外交', '出访', '峰会', '总统', '总理', '国际', '外国', '国事访问', '友好访问', '会见', '访问', '联合声明', '多边', '双边', '联合国', '一带一路', '签署', '会谈', '通话', '通电话', '晤', '元首', '首相', '国王', '女王', '天皇', '教皇', '特使', '使节'],
  ...
  economy: ['经济', '实体经济', '金融', '科技', '创新', '高质量发展', '产业', '企业', '服务业', '制造业', '数字经济', '改革开放', '自贸', '投资', '消费', '贸易', '农业', '粮食', '新质生产力', '数字化', '民营经济', '营商环境'],
  politics: ['政治', '人大', '政协', '全会', '两会', '法治', '立法', '宪法', '监察', '司法', '统一', '民族', '宗教', '港澳', '台湾'],
};

function normalizeTitleForDomain(title: string): string {
  let t = (title || '').replace(/\s+/g, ' ').trim();
  t = t.replace(/※习近平/g, '');
  t = t.replace(/^《求是》杂志发表习近平总书记重要文章[：:"《]?/, '');
  t = t.replace(/^习近平[：:]\s*/, '');
  t = t.replace(/^习近平\s+/, '');
  t = t.replace(/^(新华社|新华网|人民网|人民日报)[:：]\s*/, '');
  return t.trim();
}

export function detectDomain(title: string, category?: string): string {
  const normalized = normalizeTitleForDomain(title);

  if (category === 'call' || ['致电', '贺电', '贺信', '回信', '复信', '慰问电', '唁电'].some(kw => title.includes(kw))) {
    return 'diplomacy';
  }

  if (DOMAIN_KEYWORDS_MAP.diplomacy.some(kw => normalized.includes(kw))) return 'diplomacy';
  if (FOREIGN_LEADER_NAMES.some(name => normalized.includes(name))) return 'diplomacy';
  if (FOREIGN_COUNTRY_NAMES.some(name => normalized.includes(name))) return 'diplomacy';

  for (const [domain, keywords] of Object.entries(DOMAIN_KEYWORDS_MAP)) {
    if (domain === 'diplomacy') continue;
    if (keywords.some(kw => normalized.includes(kw))) return domain;
  }
  return 'politics';
}
