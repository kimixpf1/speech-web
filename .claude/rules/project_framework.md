...
- AI 搜索：GitHub Actions 定时/手动触发 `.github/scripts/ai_search.py`，搜索管道：直抓新华社（新华每日电讯，仅头版01）→ 直抓人民日报电子版(头版/要闻) → 直抓求是网(qstheory.cn) → 百度搜索兜底(含求是网) → 人民网讲话数据库兜底 → 统一去重(标题+URL，含URL规范化) → 与已有文章库比对(pending+articles，URL规范化去重) → 最终新增入待审核
- 自动搜索 URL 去重规范化：`normalize_url_for_dedup()` 统一 http/https、去掉 www.、去掉末尾 /、mrdx.cn/news.cn 域名规范化，避免因协议差异导致已有文章重复入库
...
- 自动搜索过滤链路：来源白名单与 URL 校验 → 新华每日电讯非头版(02/03/04版)过滤 → 评论/解读类过滤（含"总书记的人民情怀""人民论坛""人民观察""人民时评""仲音""钟声"等栏目型标题）→ 非总书记直接相关标题过滤（领会解读类、主席特使/特别代表代行出席类）→ 总书记本人活动白名单兜底（会见/出席/主持/考察/调研/致电等）→ 统一去重（标题+URL规范化）→ 历史库去重（pending+articles，URL规范化比对）→ 入待审核
...
- 自动搜索去重规则：URL 规范化去重（normalize_url_for_dedup 统一协议/www/域名）+ 标题归一化精确去重 + 待审核库与正式文章库历史标题/URL 联合去重
...
## 当前已知问题与长期待办
- 待审核列表为空问题：搜索日志显示新增1篇但前端待审核为空，可能是 Supabase 唯一约束冲突或 save_articles 静默失败（已加详细日志待下次搜索确认）
