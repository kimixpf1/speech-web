...
- 首页新增分页能力，支持每页 20/50/100/500 可选（默认 50）
- 首页分页支持页码直选与翻页后自动回到列表顶部
- 首页分页支持首页/末页跳转，并优化移动端页码按钮密度
- 修复首页/末页跳转后未稳定回顶的问题，统一分页回顶触发时机
- 增加前台部署版本可视化，便于确认线上包版本 / commit / 构建时间
- 修复 search_people_jhsjk() 的北京时间判定与日期兜底

## 本轮风险点
- ai_search.py 逻辑调整需兼容现有 workflow 入库链路
- 历史去重增加标题归一化后，需避免误杀不同文章
- 首页分页改动需保持现有筛选、搜索、滚动恢复体验不被破坏
...
- ✅ 已完成：首页增加分页与每页条数切换（20/50/100/500，默认 50）
- ✅ 已完成：首页分页增加页码直选与翻页后自动回顶
- ✅ 已完成：首页分页增加首页/末页按钮，并压缩移动端页码布局
- ✅ 已完成：修复首页/末页跳转后未稳定回顶的问题，分页回顶改为在页码更新并完成布局后执行
- ✅ 已完成：增加前台部署版本展示，显示版本号 / commit / 构建时间
- ✅ 已完成：修复 search_people_jhsjk() 的北京时间判定与日期兜底
- ✅ 已完成：build 通过
- ✅ 已完成：py_compile 通过
- ✅ 已完成：npm run lint 通过
