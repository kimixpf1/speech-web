...
- 自动搜索过滤链路：来源白名单与 URL 校验 -> 评论/解读类过滤（含“总书记的人民情怀”等栏目型标题）-> 非总书记直接相关标题过滤（领会解读类、主席特使/特别代表代行出席类）-> 总书记本人活动白名单兜底（会见/出席/主持/考察/调研/致电等）-> 统一去重（标题+URL）-> 历史库去重（pending+articles）-> 入待审核
...
- 本次补充同步了“总书记的人民情怀”栏目型误抓修复与总书记本人活动白名单兜底逻辑
