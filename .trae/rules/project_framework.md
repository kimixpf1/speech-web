...
- 首页体验优化：SWR 本地缓存回填 -> 空闲时预加载详情页 -> 列表摘要统一压缩 -> 详情返回首页恢复列表 -> 首页分页（默认每页 50，可选 20/50/100/500） -> 支持页码直选与翻页回顶 -> 支持首页/末页跳转与移动端紧凑分页 -> 分页回顶统一延后到页码状态更新后执行
...
- 自动搜索当前已在抓取阶段过滤评论、述评、解读、回响等非原文标题
