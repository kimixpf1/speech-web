...
- 首页体验优化：SWR 本地缓存回填 -> 空闲时预加载详情页 -> 列表摘要统一压缩 -> 详情返回首页恢复列表 -> 首页分页（默认每页 50，可选 20/50/100/500） -> 支持页码直选与翻页回顶 -> 支持首页/末页跳转与移动端紧凑分页 -> 分页回顶统一延后到页码状态更新后执行，并绑定目标页在布局完成后回顶 -> 顶部分页栏与快速跳页输入，减少长列表往返滚动成本
...
- src/App.tsx：首页路由、SWR 数据获取、详情页懒加载、首屏回填、分页栏、快速跳页与分页回顶控制
