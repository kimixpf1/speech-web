...
- 首页体验优化：SWR 本地缓存回填 -> 空闲时预加载详情页 -> 列表摘要统一压缩 -> 详情返回首页恢复列表
...
- 自动搜索当前已覆盖人民网讲话数据库直抓、求是网直抓、Qwen 联网搜索、Kimi 联网搜索、百度兜底
- 自动搜索当前去重规则包含 URL 规范化去重、标题归一化精确去重、文章库已有 URL 排除
- 自动搜索当前来源优先级为：求是 / 新华社 / 人民日报原文优先，人民网原文兜底
...
- 修复 search_people_jhsjk() 时区问题：当前仍存在 UTC 与北京时间差异风险
- 改进 jhsjk.people.cn 爬取：该站使用 JS 动态渲染，简单 requests.get 无法抓到完整文章列表
- 持续观察详情页拆分后的线上稳定性，重点关注摘要/解读生成、语音播报、导出 Word、返回首页链路

## 当前文档优化结论（2026-04-17）
- rules 目录已按“长期规则 / 结构框架 / 本轮待办 / 本轮迭代”四类职责重新收敛
- 本轮仅优化 .trae/rules 文档，不涉及 src、配置、构建链路或线上功能代码
- 后续若无结构变化，不必频繁改本文件；仅在核心链路、目录结构、关键职责发生变化时更新